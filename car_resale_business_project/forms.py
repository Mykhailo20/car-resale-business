...
from car_resale_business_project.models import CarMake, CarBodyType, Car, Seller, Employee, City, Purchase
...
class AddPurchaseForm(FlaskForm):
    ...
    body_type = QuerySelectField('Body_Type', query_factory=lambda: CarBodyType.query.order_by(CarBodyType.name).all(), get_label="name", allow_blank=True, blank_text="Body Type")

    manufacture_year = SelectField(choices=[(year, year) for year in range(1990, datetime.now().year + 1)], validators=[InputRequired()])
    ...
    city = QuerySelectField('City', query_factory=lambda: City.query.all(), get_label="name", allow_blank=True, blank_text="City", validators=[InputRequired()])
    ...
    purchase_date = DateField('Date', format='%Y-%m-%d', validators=[
        InputRequired(),
        DateRange(
            min=date(2010, 1, 1),
            max=date.today()

...
def extract_sale_data(conn, initial_data_loading, last_etl_datetime, limit_records=10):
    if initial_data_loading:
        ...
        query = """
            ...
            JOIN purchase p ON s.car_vin = p.car_vin
            LEFT JOIN (
                SELECT car_vin, SUM(cost) AS total_repair_cost, MAX(updated_at) AS last_repair_updated
                FROM repair
                GROUP BY car_vin
            ) r ON s.car_vin = r.car_vin
            WHERE 
                ...
                OR e.updated_at >= %s
                OR p.updated_at >= %s
                OR last_repair_updated >= %s;
        """

    with conn.cursor() as cur:
        cur.execute(query, (last_etl_datetime, last_etl_datetime, last_etl_datetime, last_etl_datetime, last_etl_datetime))
        # Fetch the results
        data = cur.fetchall()
    return data
